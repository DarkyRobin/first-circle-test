...
function createAccountTest() {
  ...
  console.assert(account.getBalance() === 100, 'Failed to initialize account with correct balance');
}
...
function overdraftTest() {
  ...
  try {
    ...
    console.error("Failed top throw error on overdraft");
  } catch (e) {
    console.assert(e.message === 'Insufficient funds.', 'Failed to handle overdraft correctly'); 
  }
}
...
function invalidDepositTest() {
  ...
  try {
    ...
    console.assert(e.message === 'Deposit amount must be positive.', 'Failed to handle invalid deposit amount correctly');
}
}
